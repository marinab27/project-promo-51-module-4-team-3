...
const addAuthorAndProject = async (req, res) => {
    ...
    const {author, job, photo, name, slogan, repo, demo, technologies, description, image} = req.body;//viajan por el body //OJO: no puedo enviar el id por el body ya que no existe aun y lo voy a crear a la vez que creo el autor

    if(!author || !job || !photo || !name || !slogan || !repo || !demo || !technologies || !description || !image /* || !author_id */){
            res.status(400).json({
    ...
    const newProject = await projectModel.addProject(name, slogan, repo, demo, technologies, description, image, author_id);//y se le pasa a la función addProject del modelo
